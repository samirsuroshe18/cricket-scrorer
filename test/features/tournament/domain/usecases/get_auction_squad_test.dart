...
class _FakeTournamentRepository implements TournamentRepository {
  ...
  @override
  Future<Either<CricketResponse<AuctionSquadRes>, CricketFailure>> getAuctionSquad({
    required String tournamentId,
  }) async {
    lastTournamentId = tournamentId;
    return Either.result(
      CricketResponse(
        message: 'Auction squad fetched',
        data: AuctionSquadRes(tournamentId: tournamentId, teams: const [], unsold: const []),
      ),
    );
  }
...
void main() {
  test('GetAuctionSquadUseCase delegates to the repository with the given tournamentId', () async {
    final repository = _FakeTournamentRepository();
    final useCase = GetAuctionSquadUseCase(tournamentRepository: repository);

    final response = await useCase(params: GetAuctionSquadParams(tournamentId: 'tournament-1'));
