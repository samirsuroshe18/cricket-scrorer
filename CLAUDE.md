...
- **No raw colors or magic numbers** in widgets — use `context.colorScheme` / `context.colors` (via `theme_x`) and the spacing extensions. Cricket-specific and severity colors are `ThemeExtension` tokens on `AppCustomColors` (`core/extensions/app_custom_colors.dart`, valued per-theme in `config/theme/palettes/app_custom_colors_palette.dart`) — reach them with `context.colors.*`. For severity/meters use `statusDanger` / `statusWarning` / `statusInfo` / `statusSuccess`; add a token to **both** `light` and `dark` rather than hardcoding a `Colors.*` value.
  ...
  - **Audited 2026-08-30**: of the 11 `Colors.white` occurrences flagged in `custom_bottomsheet.dart` (10) and `dialogue/custom_dialog.dart` (1), only one was live. `CricketLoaderDialog`'s 120×120 card (`custom_dialog.dart`) is called from 10 real sites (login, register, logout, create-match, etc.) and rendered stark white regardless of theme — fixed to `Get.theme.colorScheme.surface`, verified live on the emulator in both themes (dark: card renders theme-aware dark navy; light: card renders white, matching light `colorScheme.surface` — confirming the fix is a genuine theme read, not a hardcoded light-only value that happened to look right before). **The other 9, all in `custom_bottomsheet.dart`, sit inside factory methods nobody calls** — `womatyBottomSheet`, `womatyOptionsCustomBottomSheet`, `womatyCustomFilterBottomSheet`, `womatyCustomCityBottomSheet` (one occurrence commented out), `womatyCommonBottomSheet`, `womatyReportBottomSheet` have zero call sites anywhere in `lib/` (verified by grep) — left untouched, since editing unreachable code has no user-facing effect. The three methods actually in use (`cricketCustomBottomSheet`, `wrapBottomSheet`, `warningBottomSheet`) were already fully theme-aware before this pass. Worth a future cleanup pass to delete the dead factory methods outright, but that's a separate decision from this fix.
