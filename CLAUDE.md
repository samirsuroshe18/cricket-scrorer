...
`features/home/presentation/pages/home_dashboard_tab.dart` composes small widgets from `home/presentation/widgets/`: header, one **hero**, quiet action pills, a live-match carousel, the season stat strip and flat recent-result rows. Behaviour worth knowing before you touch it:

- **The hero is chosen, not configured.** A live/innings-break match the signed-in user is *scoring* wins (`isScoredBy` in `utils/home_match_view.dart` — an unassigned match is the caller's, an assigned one belongs to the assignee only); failing that, a created-but-unstarted one; otherwise "Start match", or the first-run walkthrough when the user has no matches at all. The hero's match is left out of the carousel so it never appears twice.
- **All derivations are display-only** and live in `utils/home_match_view.dart` (chase "need X off Y", ball-dot labels, result line via `matchResultText`, short date). Scoring truth stays server-side.
- **The status strip shows one thing at a time**: sync conflict > queued balls waiting > failed refresh. Queue/connectivity state comes from `HomeSyncStatusController`, whose sources are constructor arguments (an `OfflineSyncService.watchAllQueued()` stream and `connectivity_plus`) so tests drive it with plain streams. It is read-only — it never flushes or discards the queue.
- **Destructive/management actions are not on live cards.** Long-press (or the screen-reader custom action) opens `showMatchActionsSheet` (assign scorer, delete). The Matches tab still shows both inline on `MatchHistoryCard`.
- **"Start match" is the bottom bar's docked centre button** (`HomeBottomBar`) on every tab; there is no FAB.
- `currentInnings.battingTeam` / `target` / `recentBalls` come from `GET /v1/match/history` (see docs/api.md). All three tolerate an older server that omits them.
- The third action pill is **Search**, not Tournaments: there is no tournaments-list screen, tournaments are reached via Organizations or Search.
