...
**The scoring feature exists; the offline half does not.** `features/scoring/` scores runs, extras and wickets against the live backend, renders server-computed strike, and prompts for the next bowler at over completion — but there is still **no queue and no sync code** in `lib/`. Everything below therefore still applies to the work that remains. This section records the constraints and the open design so the first person to build it doesn't invent a fourth pattern. Offline tolerance is the highest-risk part of the product and is Phase 1 acceptance criteria, not a later pass.

**The server half is now specified.** `POST /v1/match/:matchId/sync` is defined in [docs/api.md](../docs/api.md) — an ordered, innings-scoped batch of `ball` / `bowler` / `undo` events, with a conflict check that tells a lost response apart from a foreign write. Read that section before designing anything here; the invariants it puts on the client are load-bearing, not advisory:

- **`baseAbsoluteBallSeq` is carried forward from each response**, never recomputed locally.
- **Undoing a ball that is still in the queue deletes it from the queue** and never sends it. Only an undo of an already-synced ball becomes an `undo` event. This is what stops a replayed key from re-scoring a delivery the scorer took back.
- **A batch is either all-`undo` or contains no `undo`.** Flush undos as their own batch first.
- **A batch cannot cross an innings break** — it does not carry `start-innings`, so the console has to block there until it has signal.

**Where a pending ball queues.** **Drift** is the local database — `drift`, `drift_dev`, `sqlite3_flutter_libs` and `path` sit in `pubspec.yaml` with nothing consuming them yet, kept there deliberately for this work. Don't add Hive, Isar, or a raw sqflite table alongside it. The starting point is genuinely empty: Floor was replaced by Drift, and the only thing that had used it (a `UserDetails` cache) was dead code and was deleted, so there is **no `AppDatabase`, no table, and no `lib/core/database/` at all**. The queue's schema is therefore version 1, not a migration onto an existing one. Put it under the scoring feature's `data/data_sources/local/`, matching where the deleted auth DAO lived — not in `core/`. The backend's `BallEvent` is append-only and ordered by `absoluteBallSeq`, so the local queue is an append-only log replayed in order — not a mutable "current match state" row.
...
**But the key is minted in the wrong place for a queue.** `score_ball_controller.dart` does `idempotencyKey: _uuid.v4()` inline while building the request, so it is created at the moment of the *call*, not the moment of the *tap*, and it is never stored. A retry of the same logical delivery would mint a fresh key and score the ball twice. The key has to move to where the event is enqueued and be persisted with it.

**Two `core/` gaps block reject-and-alert outright.** Neither is a scoring problem, and both need fixing before the alert can work at all:

- **`CricketFailure` has no `code` field**, and `ApiClient._handleError` never reads the `code` the backend sends. `409` isn't in its switch either, so a `SYNC_CONFLICT` arrives as a generic `CricketSomethingWentWrongFailure` — indistinguishable from any other error. Add `code` to the base class, populate it from `error.response?.data['code']`, and map `409`. Do it carefully once: `AuthInterceptor`'s refresh branching depends on the same codes and currently digs them out of raw response bodies.
- **A connection timeout is indistinguishable from a server error.** `_handleError` maps it to `CricketServerErrorFailure` with a **null** `statusCode`; only a raw `SocketException` reads as "no internet". Sync-trigger logic must not infer "we're offline" from that mapping.
